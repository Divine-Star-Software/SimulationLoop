type LoopFunction = (deltaTime: number) => void;
export const SimulationLoop = {
  ...
  _intervals: <Map<number, { delta: number; functions: Function[] }>>new Map(),
  async $INIT(speed: number) {
    this.speed = speed;
    if (performance) {
  ...
  addToInterval(interval: number, run: LoopFunction) {
